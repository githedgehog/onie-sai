...
use crate::onie_lldpd::processor::ProcessError;
use crate::onie_lldpd::processor::ProcessRequest;
...
impl onie_sai_ttrpc::OnieSai for OnieSaiServer {
    ...
    ) -> ttrpc::Result<onie_sai::LLDPNetworkConfigResponse> {
        let (tx, rx) = channel();
        self.proc_tx
            .send(ProcessRequest::LLDPNetworkConfig((req, tx)))
            .map_err(map_tx_error)?;
        let resp = rx.recv().map_err(map_rx_error)?;
        let resp = resp.map_err(map_process_error)?;
        Ok(resp)
